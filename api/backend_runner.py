...
def configure_azure_provider(credentials, quantum=False):
    credentials = ClientSecretCredential(tenant_id=credentials['AZURE_TENANT_ID'],
...
def run_job(job):
    ...
    # TODO: Removed raw_result from each call
    if job.solver.provider_name == 'PyPortfolioOptimization':
        ...
        variable_values, objective_value, time_to_solution = run_pypfopt_job(job)
...
def run_azure_qio_job(job):
    ...
    try:
        # TODO: Use full names for algorithms?
...
class Problem:
    ...
    def __post_init__(self):
        ...
        if self.resolution is not None:
            ...
            self.quadratic_problem, self.qubo_problem, self.converter = model_converter.convert_docplex_to_qubo_model(
